...
public class PartyRegistryProxyConnectorImpl implements PartyRegistryProxyConnector {
    ...
    @Override
    public InstitutionInfoCamere getInstitutionFromInfoCamereById(String externalId) {
        ProxyInstitutionResponse response = restClient.getInstitutionFromInfoCamereById(externalId);
        if(response == null){
            throw new ResourceNotFoundException(String.format(CREATE_INSTITUTION_NOT_FOUND.getMessage(), externalId),CREATE_INSTITUTION_NOT_FOUND.getCode());
        }
        return new InstitutionInfoCamere();
    }
